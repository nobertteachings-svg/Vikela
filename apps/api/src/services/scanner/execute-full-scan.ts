...
export async function executeFullScan(orgId: string): Promise<FullScanResult> {
  ...
  await prisma.scan.update({
    ...
    data: {
      ...
      passedChecks: Math.max(0, codeScans + cloudScans + identityScans - Math.min(totalGaps, 20)),
