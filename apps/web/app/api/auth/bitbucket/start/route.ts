...
/** Admin-gated Bitbucket OAuth start proxy. */
export async function GET(req: NextRequest) {
  ...
  const target = new URL(`${API_URL}/api/v1/auth/bitbucket/start`);
  ...
  dest.searchParams.set("error", body.slice(0, 120) || "Bitbucket connect failed to start");
