...
- Access to a GitLab.com (or configured self-managed) account with rights to authorize the Shieldoq OAuth app.
...
2. Sign in to GitLab if prompted and **Authorize** Shieldoq.
3. Return to Shieldoq and select projects/repos when asked.
4. Confirm **Connected**.
...
Shieldoq requests the OAuth scopes registered for your deployment (typically read API / project access sufficient to list projects and read configuration). Exact scopes are set by the platform OAuth application.
...
| Redirect URI mismatch | Operator must align GitLab app callback with Shieldoq web/API callback routes |
